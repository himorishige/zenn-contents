...
4. **`parse_agent_response_max_retries: 5`** を付けています。Llama 3.1 8B は RAG 経由で長い Observation を受けた後の Thought / Final Answer フォーマットが崩れることがあり、NAT2 記事でも採用したリトライ設定です
...
Llama 3.1 8B は RAG 経由の長い Observation 後に Final Answer フォーマットが崩れやすいです。`parse_agent_response_max_retries: 5` で救える範囲が増えます。

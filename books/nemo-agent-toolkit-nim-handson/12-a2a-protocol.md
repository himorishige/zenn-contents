---
title: "第 12 章 マルチエージェント②：A2A プロトコル"
---
...
## ReAct parse retry の不安定さ（本章で踏んだ既知課題）

実機検証では、main-agent の最終 Final Answer フェーズで ReAct の parse が何度か失敗して `ReActAgentParsingFailedError` になるケースがありました。これは第 6 章でも観察した **Llama 3.1 8B の ReAct 出力の不安定さ**が A2A 経由でも継続する現象で、A2A 固有の問題ではありません。

A2A 経由で返ってくる Observation は長文の自然言語応答（ラップされた別エージェントの Final Answer）なので、ReAct が「Thought と Final Answer を混在させて返す」失敗モードを誘発しやすいです。対処方法は 3 つあります。

- `parse_agent_response_max_retries` を 5 以上に上げる（本書採用）
- workflow_llm を Nemotron-Super-49B のような強いモデルに切り替える（NIM 無料枠で試せる）
- A2A サーバー側の `description` で応答スタイル（たとえば JSON で返す）を指定する

本書のスコープでは 1 つ目まで採用しています。**A2A 通信自体は Observation が返る時点で成功している**ので、parse 失敗が出ても「通信は通った」証拠にはなります。
...
Llama 3.1 8B で A2A + RAG の長尺 Observation を扱うときに頻発します。`parse_agent_response_max_retries: 5` 以上にする / より強いモデルに切り替える、のどちらかで改善します。

...
`-q` にコンポーネント名を渡すと、そのコンポーネントの表形式サマリ（パッケージ / バージョン / 名前 / 説明）が表示されます。どのキーが必須でどのキーが任意かを詳しく知りたい場合は、NAT のソースコードや公式ドキュメントを併読するのが確実です。
...
ReAct が 2 ツールを使い分ける出力は次のような流れになります。実測ではどちらのツールを先に呼ぶかは LLM 次第で、以下の例は `current_datetime` → `wikipedia_search` の順に呼んだケースです。
...
注目してほしいのは、ReAct が片方の Observation を読んだあとに「まだもう 1 つ残っていたな」と別のツールに切り替えている点です。ツールを並列に呼んでいるわけではなく、1 問ずつ順番に解決しています。ReAct はこのように逐次実行が前提のパターンです。

:::message
`wikipedia_search` の引数は `{"question": "..."}` 形式です。`{"query": "..."}` と書かれている記事もありますが、NAT 1.6.0 の `wiki_search` Function のスキーマは `question` がキー名になっています。LLM が渡す引数も実測で `question` が使われます。
:::

実測では Wikipedia 検索結果が必ずしも一番関連性の高いページを返すわけではありません。たとえば「NVIDIA CEO」と検索しても、`GeForce RTX 50 series` のページが上位に来て CEO 情報が含まれない、というケースも発生します。その場合も LLM は自分の事前知識で補って「Jensen Huang」と答えることが多いですが、RAG のように確実な根拠が必要な場面では第 9-10 章で扱う専用 retriever の導入が必要になります。
...
- `nat info components -t function -q <name>` で各 Function のサマリを引けるようになった

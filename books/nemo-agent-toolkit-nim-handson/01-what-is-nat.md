...
| NIM（Inference Microservice） | LLM / Embedding / Safety 等の推論エンドポイント        | クラウド NIM を全章で使用    |
...
NAT は機能がかなり広く、公式ドキュメントを読むだけだと「で、何から触ればいい？」となりがちです。本書は **「クラウド NIM でマルチエージェントの RAG アプリを一本組み上げる」という軸**でスコープを切っています。他の領域が気になる場合は、完走後に公式ドキュメントを読み込む足がかりとして使ってください。

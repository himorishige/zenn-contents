...
本書を手に取っていただきありがとうございます。クラスメソッド製造ビジネステクノロジー部の森茂と申します。
...
NAT の公式ドキュメントは網羅的ですが、初めて触る読者にとって「どこから読めばいいのか」が見えにくい構成です。本書では以下の順序で積み上げます。
...
7. MCP 連携（第 8 章）
8. RAG アドオン（第 9-10 章：FAISS → Milvus Lite）
9. マルチエージェント（第 11-12 章：agent-as-tool → A2A）
...
題材は「NVIDIA DGX Spark と NIM に関する質問に答える Q&A エージェント」です。ナレッジソースは、筆者が DevelopersIO で公開している DGX Spark 関連の記事 30 本。章を進めるごとに機能が増えていき、最終章では以下のような姿になります。

```mermaid
graph LR
    U["ユーザー質問"] --> R["Router Agent"]
...
第 2 章で Colima のインストールから丁寧にやっていきますので、Docker をこれから始める読者でもついてこられる構成です。
...
| Phoenix        | 14.8.0                                        |
| Milvus Lite    | pymilvus 2.5 系                               |

NAT はマイナーバージョンでも `_type` 名称や evaluator のカタログが変わることがあります。1.7 以降で挙動が変わった箇所は、章末にその都度差分メモを追記していく運用です。

## 読み進め方のおすすめ

- **第 1 章 → 第 3 章**は一気に読んで「Hello Agent が動く」状態を作るのをおすすめします。ここまでは無料公開しているので、NAT が自分の手に馴染むかを確かめる判断材料にしてください
...
graph TD
    ...
    C7 --> C8["第8章<br/>MCP 連携"]
    C8 --> C9["第9章<br/>RAG FAISS"]
    C9 --> C10["第10章<br/>RAG Milvus"]
    C10 --> C11["第11章<br/>agent-as-tool"]
    C11 --> C12["第12章<br/>A2A"]
    C12 --> C13["第13章<br/>nat eval"]
    C13 --> C14["第14章<br/>nat serve"]
    C14 --> C15["第15章<br/>総仕上げ"]
    style C0 fill:#fff8e1,stroke:#ff8f00
    style C1 fill:#fff8e1,stroke:#ff8f00
    style C2 fill:#fff8e1,stroke:#ff8f00
    style C3 fill:#fff8e1,stroke:#ff8f00
...
黄色が無料プレビュー、青が RAG、紫がマルチエージェント、緑が完成章です。

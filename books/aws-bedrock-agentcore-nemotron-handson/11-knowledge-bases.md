...
第 11 章では、本書の最大コスト要素である **Bedrock Knowledge Bases** を組み立てます。OpenSearch Serverless を vector store にして社内 Q&A コーパスを ingest し、`Retrieve` API で citation 付き検索を実装します。Sprint 0 で発見した「**Nemotron は Knowledge Base の generation モデルとして直接組み込み不可**」という制約を踏まえ、Agent 経由で間接 RAG する設計パターンを LangGraph state graph に組み込みます。
...
## Sprint 0 の重要制約 — Nemotron は KB に直接組み込めない

本章を進める前に、Sprint 0 で見つけた重要な制約を共有します。
...
Sprint 0 で確認した OpenSearch Serverless 以外の vector store の整理を再掲します。
...
次章は **Bedrock Guardrails** です。Sprint 0 で見つけた決定的な発見、「**Classic tier では日本語 content filter / PII / Prompt Attack が動かない、Standard tier + APAC profile + Cross-Region Inference 必須**」を冒頭に置き、日本語入出力検閲の実機検証ログを丁寧に追います。PII Email / Phone が Standard tier でも検出されない限界も正直に書き、NemoGuard Safety Guard NIM を補助で併用するパターンに踏み込みます。

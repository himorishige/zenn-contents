...
第 12 章は本書の中で **Sprint 0 の発見がもっとも濃く反映される章**です。Bedrock Guardrails をデフォルトで作ると日本語の Content filter / PII / Prompt Attack はほぼ動かない、という事実から始め、Standard tier に切り替えると劇的に改善する流れ、そして PII Email / Phone は Standard tier でも検出できないという限界まで、実機検証ログを丁寧に追います。NeMo Guardrails の Multilingual Safety Guard NIM を補助で併用するパターンに踏み込み、本書から消したかった NeMo 連続性の最後の砦を据えます。
...
## Sprint 0 の決定的発見 — Classic tier では日本語が動かない

本章の冒頭で、**本書の Sprint 0 で見つけた最大の落とし穴**を共有します。
...
Sprint 0 で日本語のテストプロンプトを Classic tier の Guardrail に投げたところ、次の結果でした。
...
**6 件中 3 件が日本語で検出失敗**。Topic（自然言語定義）と Regex は機能しましたが、Content filter / PII / Prompt Attack は完全にスルーされました。

英語で同じテストを投げると content filter / Prompt attack は普通に動くので、これは「Classic tier が言語に依存している」という構造的な制約です。

### Standard tier に切り替えると劇的に改善

Sprint 0 で同じ Guardrail を Standard tier に切り替えると、結果は次のようになりました。
...
データ越境を許容できない要件の場合は、Standard tier の利用を見直す必要があります。Sprint 0 で AWS Documentation MCP から確認した正式情報です。
...
Standard tier でも、**PII Email / Phone は日本語コンテキストで検出されない**のが Sprint 0 で見つけた限界です。
...
シンプルだが効きます。Sprint 0 PoC 5 で「マイナンバー regex は Classic tier でも動いた」という事実があるので、ML 検出に頼らず regex で確実に止める設計が現実解です。
...
ただし Sprint 0 で実機検証していないので、本番投入前にテストデータで閾値調整が必要です。Ch 13（評価）でこの調整プロセスを扱います。

## トラブルシューティング

### 「Can't configure guardrail policy tier. Enable cross-Region inference」

Standard tier に切り替えるときに `crossRegionConfig` を指定し忘れています。Sprint 0 でも踏んだエラーで、`apac.guardrail.v1:0` を追加すれば解消します。
...
- **Sprint 0 の核心：Classic tier では日本語 content filter / PII / Prompt Attack が動かない**
...
次章は **Evaluations**（評価）です。Bedrock Model Evaluation、Knowledge Base Evaluation、AgentCore Evaluator、自作 Nemotron Nano 9B v2 LLM-as-Judge の 4 系統を比較し、本書の社内 Q&A シナリオで現実的な評価サイクルを組みます。Sprint 0 で見つけた **AgentCore Evaluations の Tier1 Output が $12 / 1M tokens で高価**という制約を踏まえ、サンプルサイズ設計と評価コスト最適化のパターンも扱います。

...
推論モデルは **Bedrock ネイティブの Nemotron Nano 3 30B** を主軸に据えます。東京リージョンで日本語応答が 1 秒未満、単価は Claude Sonnet 4.5 の 1/40 〜 1/100 という現時点のスイートスポットです。
...
- **SageMaker Endpoint 経由の NIM デプロイ**は実機検証で撤退済みのため、本書では扱いません（CUDA Graph timeout / `serve` スクリプト不在で全試行 Failed）
...
:::message
**コスト感**: 月 1,000 conversation 想定で、Knowledge Bases なしの開発環境なら月 10 USD 程度、本番想定で Knowledge Bases を常時起動すると月 367 USD 程度です。OpenSearch Serverless の OCU 固定費（約 345 USD/月）が支配要因なので、開発時は Knowledge Bases を停止できる CDK パラメータを第 11 章で実装します。
:::
...
- **付録 A** は前作読者向けの差分マップ、**付録 B** は東京以外で動かす場合の考慮点です

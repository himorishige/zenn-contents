...
前章で `qaSupervisor` プロジェクトを `agentcore deploy` で AWS に上げ、Runtime での invoke を確認しました。本章ではその `qaSupervisor` に Memory を後から追加します。Sprint 0 で `agentcore add memory --strategies SEMANTIC --expiry 7 --json` を実行して project config に `qaShortTermMem` を追加するところまで済んでいるので、本章ではそれを実 AWS リソースとしてデプロイする段階から始められます。
...
すでに Sprint 0 で実施したので、コマンドだけ再掲します。
...
過去の事実（Long-Term Memory）を system prompt に注入することで、ユーザー固有のコンテキストを引き継いだ応答ができます。Sprint 0 段階では Memory の動作確認まで済んでいないので、ここから先のコード断片は本書のサンプルリポで実機検証しながら詰めていく形になります。
...
次章は **AgentCore Gateway** です。Lambda で実装した社内 HR API モックを Gateway 経由で MCP 互換ツールに変換し、LangGraph から呼び出せるようにします。OpenAPI スキーマからの自動生成、認証連携（IAM / Cognito JWT）、MCP の世界観と LangChain tool の境界、そして既存 REST API を MCP 化する一般化パターンまで扱います。Sprint 0 では「Gateway API が東京で利用可能」までしか確認していないので、Sprint 3 で初めての実機構築になります。

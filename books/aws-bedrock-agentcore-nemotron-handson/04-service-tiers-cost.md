...
第 4 章では、Bedrock の **Service Tiers**（Standard / Priority / Flex / Batch）の挙動と、本書の社内 Q&A エージェントを月いくらで動かせるかの試算を、実数値ベースで整理します。コストの章は飛ばされがちですが、AgentCore Runtime や Knowledge Bases は気を抜くと月額が想定の 2 〜 3 倍になりやすいので、ハンズオンに入る前にコスト感を一度押さえておくことを強くおすすめします。
...
前章で Bedrock ネイティブ Nemotron を実機で叩き、Nano 3 30B が東京で 835ms で日本語応答を返すことを確認しました。本章ではその実測値をもとに、月額のコスト試算に踏み込みます。コストの数値は 2026-04 時点の AWS Pricing API から取得しました。
...
東京リージョンでの単価は次の通りです（AWS Pricing API 取得）。
...
AgentCore は Runtime / Memory / Gateway / Identity / Built-in Tools / Observability / Evaluator / Policy Engine の 8 領域で個別課金されます。東京リージョンの単価は次の通りです。
...
一方、**Evaluations の Tier1 Output が $12 / 1M tokens** で、Bedrock の Nano 3 30B 出力単価（$0.35 / 1M tokens）の **34 倍**もします。第 13 章で AgentCore Evaluator を使うときはサンプルサイズと評価回数を慎重に設計する必要があります。
...
Knowledge Bases を本番並みに常時起動すると、月額の **94%** が OpenSearch Serverless OCU で消えます。本書の第 11 章では、CDK スタックに「KB を停止できるパラメータ」を実装し、dev 環境では明示的に停止できる構成にします。`cdk deploy --context kb-enabled=false` のように切り替える設計です。

## 月額試算シナリオ

ここまでの単価を組み合わせて、3 つのシナリオで試算します。
...
設定 CDK スタックは第 16 章で扱うので、ここでは概念だけ押さえてください。

## 章末まとめ

- Bedrock Service Tiers は Standard / Priority / Flex / Batch の 4 種、開発は Flex、本番は Standard が定番
- Nemotron Nano 3 30B Standard 単価は入力 $0.07 / 1M、出力 $0.35 / 1M で、Claude Sonnet 4.5 の約 43 分の 1
- 月 1,000 conversation シナリオの月額試算： dev **$10** / staging **$190** / prod **$367**
- prod 月額の 94% は OpenSearch Serverless OCU。第 11 章で停止 CDK パラメータを実装する
- AgentCore Evaluations の Tier1 Output は $12 / 1M で高価、サンプルサイズ設計が必須

ここまでで、手元に Bedrock model access 有効化済みの環境があり、Nemotron を Converse API で叩け、月額の見通しも立っている状態が揃いました。次章から Part 3「AgentCore 6 サービスを順に積む」に入ります。

## 次章では

次章は、本書の核である **AgentCore Runtime に LangGraph をデプロイする**ハンズオンです。`agentcore create` で scaffold したプロジェクトに Nemotron Nano 3 30B を組み込み、ローカル `agentcore dev` で動作確認した後、`agentcore deploy` で AWS にデプロイします。

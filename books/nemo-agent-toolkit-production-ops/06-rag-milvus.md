...
意図したファイル（`faq/01-expense-faq.md`）が rank 2 でヒットしています。rank 1 に「休暇取得 FAQ」が来たのは、両ファイルとも「申請」「締切」のような共通語彙が多くて、ベクトル空間で近接した結果です。本書の主題は retrieval 精度の追い込みではないので、ここは「ベクトル検索だけだと多少のばらつきがある」と割り切って先に進みます。
...
これも retrieval だけでは絞り込みきれない、という現実そのものです。第 7 章では LangGraph の中で「`category=='department-notes'` を優先的に絞ってから類似度検索する」のような重み付けを入れて、PII 含む chunk を意図的に取りに行く構成を試します。retrieval を「ただベクトル類似度を見る道具」として使うのではなく、「メタデータでスコープを絞ってから類似度を見る道具」として使う発想の転換が、本書ではキモになります。

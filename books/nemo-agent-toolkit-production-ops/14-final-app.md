...
本章は第 7-13 章で個別に動かした要素を、最後に 1 つの統合システムにまとめる総決算です。新しい部品は登場しませんが、これまで分散していた装備が 1 つの compose で同時に効く様子は、本書を通読する醍醐味のひとつです。途中の章をスキップして本章にきたみなさんは、第 7 章（RAG）と第 9 章（Multilingual Safety Guard）と第 12 章（Prompts）の 3 つだけは戻って眺めてから、本章の構成図に進んでください。
...
ここまでで 4 本柱の統合は完了です。次の第 15 章では、本章で作った構成にもう一段踏み込んで、retrieve ノードを **3 つの専門エージェント（FAQ / IT セキュリティ / 部署ディレクトリ）に分解する supervisor pattern** を扱います。本書のスコープを少し超える拡張ですが、production で「FAQ プロンプトを直したら IT セキュリティの応答品質が落ちた」のような副作用を防ぐには、自然に登場する設計パターンです。

第 15 章のあとには、付録 A（Phoenix → Langfuse 移行ガイド）と付録 B（Langfuse Cloud と self-hosted の使い分け + 本書の締めくくり）が続きます。

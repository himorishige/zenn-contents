...
第 15 章は、第 14 章で組んだ 5 ノード統合構成を、もう一段拡張する章です。第 7 章から第 14 章まで、本書のエージェントは「**1 つの retrieve ノードがすべての文書を引く**」前提で組んできました。本章ではその retrieve 部分を **3 つの専門エージェントに分解** し、supervisor が質問を見てどれに任せるかを判定する構成に作り変えます。

LangGraph の **Supervisor pattern** は、production 環境でエージェントを育てる過程で自然と登場するパターンで、本書の 4 本柱の上にそのまま乗せられる拡張です。第 1 章で「LangGraph 単体で multi-step を表現する」と宣言したスコープから少し踏み出しますが、フレームワークは LangGraph の中で完結するので、本書の枠組みは保たれます。
...
`_expert_run` の system prompt にエージェント名を埋め込んでいるのがミソです。`f"あなたは Example 株式会社の {expert_name} です。"` の形で、Nemotron Super 49B が「自分は何を専門に答えるエージェントなのか」を意識して回答するようにしています。

第 7 章の `retrieve_node` との差は、**フィルタとプロンプトが expert 単位で独立する** ことだけで、Milvus への問い合わせ手順や `output_fields` の指定は同じです。Ch 7 の retrieve 実装が分かっていれば、本章は「1 つの retrieve を 3 つに増やしてフィルタを散らした」と読み替えられます。
...
| 2. Guardrails    | 第 14 章と同じく **supervisor の前** に input rail、**finalize の後** に output rail を置く。expert 単位ではなく workflow 単位で守る |
| 3. Observability | Langfuse trace に supervisor → expert → finalize の分岐が並び、Agent Graph で構造が見える                                            |
| 4. Eval Dataset  | Datasets Run で「どの expert が呼ばれたか」を score として記録し、supervisor の判定精度を継続評価できる                              |
...
3 つ目は **expert の system prompt の管理** です。本章では学習しやすさを優先して、エージェント名を `f"あなたは Example 株式会社の {expert_name} です。"` で埋め込みました。本章のサンプルとしてはこれで通りますが、運用に乗せる際は第 12 章の Langfuse Prompts に **expert ごとのプロンプトを別 name で登録** するのが次の一手です。`internal-qa-faq-expert` / `internal-qa-security-expert` のようなプロンプト名にしておけば、専門ごとに独立してバージョン管理 + A/B テストができます。本章のサンプルリポを土台に、第 12 章の `langfuse.get_prompt(...)` を expert ごとに呼び分ける改造に進むのが、production 移行の自然なステップです。

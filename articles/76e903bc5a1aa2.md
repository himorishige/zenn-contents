...
:::message alert
React Query v4.0.0-beta.9よりQuery Functionからundefinedを返すことができなくなりました。記事のソースコードではbeta.9〜10で型エラーが出てしまいます。issueとして報告済みのため近々に解消されると思われます。なお一時的にエラーを回避するため記事内のサンプルコードを当初の形から明示的に`unknown`を渡す形へ一部書き換えを行っています。
